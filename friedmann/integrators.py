...
class CumulativeSumIntegrator:
    ...
    def __init__(self, Omega_r, Omega_m, H0):
        ...
        self._hubble_distance = 2997.9246 #Mpc/h
    ...
    def _integrate(self):
        ...
        spacing = np.diff(self._a, prepend = 0)
        ...
        self._tH0 = np.cumsum(spacing/ np.sqrt(self._Omega_r/self._a**2+self._Omega_m/self._a+self._Omega_lam*self._a**2))
        #2 / 3 *self._a ** (3 / 2)
        self._etaH0 = np.cumsum(spacing/ np.sqrt(self._Omega_r+self._Omega_m*self._a+self._Omega_lam*self._a**4))
        #2 * np.sqrt(self._a)
    ...
    def hubble_parameter(self):
        """This is wrong unless Omega_m = 1."""
        return self.H0*np.sqrt(self._Omega_r/self._a**4+self._Omega_m/self._a**3+self._Omega_lam) #/ self._a**2
    ...
    def proper_distance(self):
        return self._hubble_distance * (self._tH0[-1] - self._tH0)

    @property
    def luminocity_distance(self):
        return  self._hubble_distance * (self._etaH0[-1] - self._etaH0)/self._a
