...
def test_matter_dominated():
    ...
    assert integ._hubble_distance == pytest.approx(HUBBLE_DISTANCE, 0.1)
...
def test_calc():
    integ = integrators.CumulativeSumIntegrator(5e-5, 0.3, 100)
    np.savetxt('output.dat', (integ.a,integ.z,integ.comoving_distance, integ.proper_distance, integ.luminocity_distance,integ.hubble_parameter))
